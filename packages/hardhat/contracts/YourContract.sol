...
contract YourContract is VRFConsumerBaseV2, ERC721URIStorage {
    ...
    event Minted(uint256 _tokenId, address sender, string indexed mission);
    /**
     ...
     */
    constructor(uint64 subscriptionId) VRFConsumerBaseV2(vrfCoordinator) ERC721("YourAvatar", "DNDD") {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        s_owner = msg.sender;
        s_subscriptionId = subscriptionId;
    }

    function mintNFT(string memory mintURI) public {
        uint256 newAvatarId = _tokenIds.current();
        _mint(msg.sender, newAvatarId);
        _setTokenURI(newAvatarId, mintURI);
        
        // call the rollDice function?
        rollDice(msg.sender);

        // lets
        emit Minted(newAvatarId, msg.sender, mission(msg.sender));
        _tokenIds.increment();
    }
